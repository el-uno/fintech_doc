...
Solana is the useful case for seeing the difference, because it exposes finality as **progressive commitment levels** rather than a single block count. A transaction is processed, then _confirmed_ once a supermajority of validators has voted on its block, then _finalized_ once enough confirmed blocks are rooted above it. Roughly 32 slots at 400ms reaches finalized, which is where the ~13 second figure in the table below comes from.
